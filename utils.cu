...
struct cal_transform{
    ...
    explicit cal_transform(int* loop_size,const Eigen::Vector3f& angular_init_pose, const int& angular_winsize, const float& angular_step,
                           const Eigen::Vector3f& linear_init_pose, const int& linear_winsize, const float& linear_step):_loop_size(loop_size),
                           _angular_init_pose(angular_init_pose),_angular_winsize(angular_winsize),_angular_step(angular_step),
...
struct get_element3{
    int x_;
    explicit get_element3(int x):x_(x){}
...
struct get_element4{
    int x_;
    explicit get_element4(int x):x_(x){}
...
std::vector<int> MinMaxXYZ(thrust::device_vector<Eigen::Vector4f>& dev_map, int map_size){
    ...
    thrust::transform(thrust::device, dev_map.begin(), dev_map.end(), point_x.begin(), get_element4(0));
    thrust::transform(thrust::device, dev_map.begin(), dev_map.end(), point_y.begin(), get_element4(1));
    thrust::transform(thrust::device, dev_map.begin(), dev_map.end(), point_z.begin(), get_element4(2));
    cudaDeviceSynchronize();
...
{
    __host__ __device__
    float operator()(Eigen::Vector4f map_element)
    {
        return map_element[3];
    }
};
...
struct faster_compute_point_score:public thrust::unary_function<Eigen::Vector3f, float> // <arg, result>
{
    ...
    float operator()(Eigen::Vector3f& point)
    {
        ...
        int key = (int(transformed_point[0])+500)*1000*1000+(int(transformed_point[1])+500)*1000+(int(transformed_point[2])+500);

        if(fabs(_map_odds[key]-10000.0)<1e-6)
        {
            return 0.0;
        }
        else
...
//        TODO:判断transformed_point的xyz的范围是否在合理的范围内,否则生成的key不可靠,如此即可省去key的判断
        int key = (int(transformed_point[0])+_offset[0])*_map_length[1]*_map_length[2]+(int(transformed_point[1])+_offset[1])*_map_length[2]+(int(transformed_point[2])+_offset[2]);

        if(key<0 || key >=_map_odds_size)
            return 0.0;
        if(fabs(_map_odds[key]-10000.0)<1e-6)
        {
            return 0.0;
...
//    int pose_num = poses.size();
    ...
    thrust::fill(thrust::device, map_odds.begin(), map_odds.end(), 10000.0);
    cudaDeviceSynchronize();
...
//                                                                                                                       thrust::raw_pointer_cast(map_odds.data()), map_size, map_resolution));
    thrust::transform(thrust::device, transforms.begin(), transforms.end(), scores.begin(), faster_compute_cloud_scoreV2(thrust::raw_pointer_cast(dev_scan.data()), scan_size,
                                                                                                                       thrust::raw_pointer_cast(map_odds.data()), map_size, map_resolution,
                                                                                                                         thrust::raw_pointer_cast(dev_offset.data()), thrust::raw_pointer_cast(dev_maplength.data()),
                                                                                                                         map_odds_size));
    cudaDeviceSynchronize();
